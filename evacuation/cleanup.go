...
func (e *EvacuationCleanup) Run(signals <-chan os.Signal, ready chan<- struct{}) error {
	...
	actualLRPGroups, err := e.bbsClient.ActualLRPGroups(nil, logger, models.ActualLRPFilter{CellID: e.cellID})
	if err != nil {
	...
	for _, group := range actualLRPGroups {
		...
		err = e.bbsClient.RemoveEvacuatingActualLRP(nil, logger, &actualLRP.ActualLRPKey, &actualLRP.ActualLRPInstanceKey)
		if err != nil {
